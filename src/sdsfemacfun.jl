...
```
"""
macro frontier(constanttype,arg::Vararg )  # for sfmodel_spec() 
    if constanttype ∈ (:constant,:constant)
        return (:frontier, unique(vcat(:_consssssss,collect(:($(arg))))))
    else
        return (:frontier, unique(collect(:($(arg)))))
    end
 end 
...
"""
 ...
 macro frontierWx(arg::Vararg)   # for sfmodel_spec()          
    return (:frontierWx, collect(:($(arg))))
 end 
...
macro envar(arg::Vararg)   # for envar()          
    return (:envar, collect(:($(arg))))
end 

 macro ivvar(arg::Vararg)   # for envar()          
    return (:ivvar, collect(:($(arg))))
end 
...
function wx(arg::Matrix)
    return (:wx, arg)
end
function wx()
    return (:wx, Nothing)
end
function wy(arg::Matrix)
    return (:wy, arg)
end
function wy()
    return (:wy, Nothing)
end
function wu(arg::Matrix)
    return (:wu, arg)
end
function wu()
    return (:wu, Nothing)
end
function wv(arg::Matrix)
    return (:wv, arg)
end
function wv()
    return (:wv, Nothing)
...
margeffu(arg::Bool)
 ...
 compute the margeffu effects of the exogenous determinants of inefficiency (if any).
 
 See the help on `sfmodel_opt()` for more information.
  
 # Examples
 ```julia-repl
 sfmodel_opt( margeffu(false), ...)
 ```
""" 
 function margeffu(arg::Bool=true)
    return (:margeffu, arg)
 end
...
function cfindices(arg=nothing)
    ...
    if !(isa(arg, Dict))
        throw("The keyword of `cfindices` in `sfmodel_opt()` is specified incorrectly. Only allow Dict. Got `$(arg)` instead.")
    end 
